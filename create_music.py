...
def main():
    ...
    print("    uint8_t note: 5, time: 3;")
    print("} _note;")
    print
    print("typedef union {")
    print("    _note;")
    print("    uint8_t decode;")
    print("} note;")
    print
    print("const uint8_t note_clocks[] = {")
    ...
    print("};")
    print
    print("const _note tune[] PROGMEM = {")
    track_notes = [event for event in track if isinstance(event, midi.NoteOnEvent)]
    note_ticks = {event.tick for event in track_notes}
    note_gcd = reduce(gcd, note_ticks)
    print("\n".join("    {%d, %d}, // %s" % (note_map[event.pitch],
                                             (nextevent.tick / note_gcd) - 1,
    ...
    print("#define NOTE_LENGTH_MULTIPLIER %d" % note_gcd)
    print
    print("#endif // TUNE_H")
